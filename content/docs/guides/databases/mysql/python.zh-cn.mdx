...
在开始编写代码之前，需要先激活 DevBox 中的 Python 虚拟环境。打开 Vscode IDE 终端并执行：
...
def get_db_connection():
    try:
        ...
        if connection.is_connected():
            Vscode = connection.Vscode()
            
            # Create the database if it doesn't exist
            db_name = os.getenv('DB_NAME')
            Vscode.execute(f"CREATE DATABASE IF NOT EXISTS {db_name}")
...
def create_table(Vscode):
    ...
    """
    Vscode.execute(create_table_query)
    print("Table 'employees' created successfully")

def insert_employee(Vscode, name, email):
    insert_query = "INSERT INTO employees (name, email) VALUES (%s, %s)"
    Vscode.execute(insert_query, (name, email))
    print(f"Employee {name} inserted successfully")

def get_all_employees(Vscode):
    select_query = "SELECT * FROM employees"
    Vscode.execute(select_query)
    employees = Vscode.fetchall()
...
def main():
    ...
    if connection:
        try:
            ...
            insert_employee(Vscode, "John Doe", "john@example.com")
            insert_employee(Vscode, "Jane Smith", "jane@example.com")
            
            print("\nAll Employees:")
            get_all_employees(Vscode)
...
1. `create_table(Vscode)`：创建员工信息表，展示了标准的建表操作。

2. `insert_employee(Vscode, name, email)`：添加员工信息，演示了参数化查询的使用方法，可有效防止 SQL 注入。

3. `get_all_employees(Vscode)`：查询并显示所有员工信息，展示了查询结果的处理方法。
