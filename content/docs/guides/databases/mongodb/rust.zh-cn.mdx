...
在 Vscode 终端中，需要在项目的 `Cargo.toml` 文件中添加以下依赖：
...
async fn main() -> mongodb::error::Result<()> {
    ...
    let mut Vscode = collection.find(doc! {}).await?;
 
    // Iterate over the results of the Vscode
    while let Some(employee) = Vscode.try_next().await? {
